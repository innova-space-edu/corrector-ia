...
class _UploadScreenState extends State<UploadScreen> {
  ...
  enum Step { name, upload, done }
  Step _step = Step.name;
  ...
  Future<void> _start() async {
    ...
    try {
      ...
      setState(() { _step = Step.upload; _loading = false; });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(child: () {
        switch (_step) {
          case Step.name: return _nameView();
          case Step.upload: return _uploadView();
          case Step.done: return _doneView();
        }
      }()),
    );
  }
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(
            children: _slots.asMap().entries.map((e) {
              ...
              Color c;
              if (i == _current) c = const Color(0xFF2563EB);
              else if (s.status == SlotStatus.done) c = const Color(0xFF22C55E);
              else if (s.status == SlotStatus.skipped ||
                  s.status == SlotStatus.error) c = const Color(0xFFF97316);
              else c = const Color(0xFFE5E5E4);
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text('Ejercicio ${_current + 1} de ${_slots.length}',
                  style: const TextStyle(fontSize: 12, color: Color(0xFF78716C))),
              const SizedBox(height: 3),
              Text(
                slot.statement.isNotEmpty ? slot.statement : 'Sube la foto de este ejercicio',
  ...
  Widget _doneView() {
    final done = _slots.where((s) => s.status == SlotStatus.done).length;
    final skip = _slots.where((s) =>
        s.status == SlotStatus.skipped || s.status == SlotStatus.error).length;
    ...
    return Center(child: Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text(
          ...
          style: const TextStyle(fontSize: 13, color: Color(0xFF78716C), height: 1.6),
        ),
        const SizedBox(height: 10),
        Container(
          padding: const EdgeInsets.all(12),
